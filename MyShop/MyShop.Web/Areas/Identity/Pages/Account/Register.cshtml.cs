...
namespace MyShop.Web.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        {
            //if role of admin doesnt exist
            if(!_roleManager.RoleExistsAsync(SD.AdminRole).GetAwaiter().GetResult())
            {
                //create role when open register page (Admin - Editor - Customer)
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    //check role (Customer or Admin) from register page
                    string role = HttpContext.Request.Form["RoleRadio"].ToString();
                    if(string.IsNullOrEmpty(role))
                    {
                        //Customer who is login
                        await _userManager.AddToRoleAsync(user, SD.CustomerRole);
                        // it show hello email@gmail.com 
                        await _signInManager.SignInAsync(user, isPersistent: false);
                        return LocalRedirect(returnUrl);
                    }
                    else
                    {
                        //Admin who is login and his admin that select from 3 radio(Admin - Editor - Customer)
